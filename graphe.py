...
class Sommet:
    def __init__(self, nom):
        self.nom = nom
        self.successeurs = {}  # Dictionnaire {sommet: (capacité, poids)}
        self.predecesseurs = {}  # Dictionnaire {sommet: (capacité, poids)}
    
    def ajouter_successeur(self, sommet, capacite, poids=0):
        self.successeurs[sommet] = (capacite, poids)
        sommet.predecesseurs[self] = (capacite, poids)
...
class Graphe:
    ...
    def ajouter_arete(self, source, destination, capacite, poids=0):
        if source in self.sommets and destination in self.sommets:
            self.sommets[source].ajouter_successeur(self.sommets[destination], capacite, poids)

    def supprimer_arete(self, source, destination):
        if source in self.sommets and destination in self.sommets:
            dest_obj = self.sommets[destination]
            src_obj = self.sommets[source]
            if dest_obj in src_obj.successeurs:
                del src_obj.successeurs[dest_obj]
            if src_obj in dest_obj.predecesseurs:
                del dest_obj.predecesseurs[src_obj]

    def update_arete(self, source, destination, nouvelle_capacite=None, nouveau_poids=None):
        if source in self.sommets and destination in self.sommets:
            src = self.sommets[source]
            dest = self.sommets[destination]
            if dest in src.successeurs:
                cap, p = src.successeurs[dest]
                cap = nouvelle_capacite if nouvelle_capacite is not None else cap
                p = nouveau_poids if nouveau_poids is not None else p
                src.successeurs[dest] = (cap, p)
                dest.predecesseurs[src] = (cap, p)

    def afficher(self):
        for sommet in self.sommets.values():
            print(f"Sommet {sommet.nom}:")
            print(f"  Successeurs: {[(s.nom, c, p) for s, (c, p) in sommet.successeurs.items()]}")
            print(f"  Prédécesseurs: {[(s.nom, c, p) for s, (c, p) in sommet.predecesseurs.items()]}")
            print()
